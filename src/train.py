...
from config import *
from loss import *
from image_transform import load_image
...
def run():
    ...
    train_x = train_x.map(load_image, num_parallel_calls=AUTOTUNE)
    train_x = train_x.shuffle(buffer_size=BUFFER_SIZE)
    #train_x = train_x.repeat()
    train_x = train_x.batch(BATCH_SIZE)
    train_x = train_x.prefetch(buffer_size=AUTOTUNE)

    train_y = train_y.map(load_image, num_parallel_calls=AUTOTUNE)
    train_y = train_y.shuffle(buffer_size=BUFFER_SIZE)
    #train_y = train_y.repeat()
    train_y = train_y.batch(BATCH_SIZE)
    train_y = train_y.prefetch(buffer_size=AUTOTUNE)

    test_y = test_y.map(load_image, num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE)

    sample_photo = next(iter(test_y))
    ...
    for epoch in tqdm(range(EPOCHS), desc="Epochs"):
        ...
        for image_x, image_y in tqdm(tf.data.Dataset.zip((train_x, train_y)), desc="Dataset iterator", total=1072):
            train_step(image_x, image_y)

        generate_images(generator_g, sample_photo, epoch)
